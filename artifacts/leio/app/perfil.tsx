...
function TrendChart({ data, width, color, fill, axisColor, labelColor }: TrendChartProps) {
  const height = 160;
  const padLeft = 8;
  const padRight = 44;
  const padTop = 12;
  ...
  const areaPath =
    pts.length > 0
      ? `${path} L ${pts[pts.length - 1].x} ${padTop + chartH} L ${pts[0].x} ${padTop + chartH} Z`
      : "";
  ...
  return (
    <Svg width={width} height={height}>
      <SvgLine x1={padLeft} x2={padLeft + chartW} y1={yLineFull} y2={yLineFull} stroke={axisColor} strokeWidth={1} />
      <SvgLine x1={padLeft} x2={padLeft + chartW} y1={yLineHalf} y2={yLineHalf} stroke={axisColor} strokeWidth={1} opacity={0.4} />
      <SvgLine x1={padLeft} x2={padLeft + chartW} y1={yLineZero} y2={yLineZero} stroke={axisColor} strokeWidth={1} opacity={0.4} />
      <SvgText x={padLeft + chartW + 6} y={yLineFull + 4} fill={labelColor} fontSize={10} fontWeight="600">{`${niceMax} pág`}</SvgText>
      <SvgText x={padLeft + chartW + 6} y={yLineHalf + 4} fill={labelColor} fontSize={10} fontWeight="600">{`${half} pág`}</SvgText>
      <SvgText x={padLeft + chartW + 6} y={yLineZero + 4} fill={labelColor} fontSize={10} fontWeight="600">0</SvgText>
      {areaPath !== "" && <Path d={areaPath} fill={fill} />}
      {path !== "" && <Path d={path} stroke={color} strokeWidth={2} fill="none" />}
      {monthLabels.map((m, i) => (
        <SvgText key={i} x={m.x} y={height - 6} fill={labelColor} fontSize={10} fontWeight="700" textAnchor="middle">{m.label}</SvgText>
      ))}
    </Svg>
  );
}
...
function MonthlyBars({ sessions, year, month, width, height, color, mutedColor }: MonthlyBarsProps) {
...
export default function PerfilModalScreen() {
  ...
  const bestSession = sessions.reduce((max, s) => Math.max(max, pagesOf(s)), 0);
  ...
  const thisWeekPages = thisWeekSessions.reduce((acc, s) => acc + pagesOf(s), 0);
  const thisWeekTime = thisWeekSessions.reduce((acc, s) => acc + s.durationSeconds, 0);
  ...
  return (
    <ScrollView
      ...
      contentContainerStyle={[
        styles.content,
        { paddingTop: topInset + 12, paddingBottom: 40 + bottomInset },
      ]}
    ...
    >
        ...
        <View style={styles.profileInfo}>
          ...
          <View style={[styles.xpBar, { backgroundColor: colors.border }]}>
            <View
              style={[
                styles.xpFill,
                { backgroundColor: colors.volt, width: `${Math.round(progress * 100)}%` },
              ]}
            />
          </View>
      ...
      <View style={styles.section}>
        ...
        <View style={[styles.weekTrioCard, { backgroundColor: colors.card, borderColor: colors.border }]}>
          <View style={styles.weekTrio}>
            <View style={styles.weekTrioItem}>
              <Text style={[styles.weekTrioLabel, { color: colors.mutedForeground }]}>Páginas</Text>
              <Text style={[styles.weekTrioValue, { color: colors.foreground }]}>{thisWeekPages}</Text>
            </View>
            <View style={styles.weekTrioItem}>
              <Text style={[styles.weekTrioLabel, { color: colors.mutedForeground }]}>Tempo</Text>
              <Text style={[styles.weekTrioValue, { color: colors.foreground }]}>{formatDuration(thisWeekTime)}</Text>
            </View>
            <View style={styles.weekTrioItem}>
              <Text style={[styles.weekTrioLabel, { color: colors.mutedForeground }]}>Sessões</Text>
              <Text style={[styles.weekTrioValue, { color: colors.foreground }]}>{thisWeekCount}</Text>
            </View>
          </View>
          <Text style={[styles.trendLabel, { color: colors.mutedForeground, marginTop: 18 }]}>
            Últimas 12 semanas
          </Text>
      ...
      <View style={styles.section}>
        ...
        <View style={styles.bests}>
          {[
            ...
            <View key={i} style={[styles.bestCard, { backgroundColor: colors.card, borderColor: colors.border }]}>
              <Ionicons name={item.icon as never} size={20} color={colors.accentText} />
              <Text style={[styles.bestValue, { color: colors.foreground }]}>{item.value}</Text>
              <Text style={[styles.bestLabel, { color: colors.mutedForeground }]}>{item.label}</Text>
            </View>
          ))}
        </View>
      </View>
        ...
        <View style={styles.monthHeaderRow}>
          ...
          <View
            style={[
              styles.streakPill,
              {
                backgroundColor: streakWeeks > 0 ? `${colors.volt}22` : colors.secondary,
                borderColor: streakWeeks > 0 ? colors.accentBorder : colors.border,
              },
            ]}
          >
            <Ionicons name="flame" size={12} color={streakWeeks > 0 ? colors.accentText : colors.mutedForeground} />
            <Text style={[styles.streakPillText, { color: streakWeeks > 0 ? colors.accentText : colors.mutedForeground }]}>
        ...
        <View style={[styles.calendarCard, { backgroundColor: colors.card, borderColor: colors.border }]}>
          <View style={styles.calendarStatsRow}>
            <View style={styles.calendarStat}>
              <Text style={[styles.calendarStatLabel, { color: colors.mutedForeground }]}>Sua sequência</Text>
              <Text style={[styles.calendarStatValue, { color: colors.foreground }]}>
                {streakWeeks} {streakWeeks === 1 ? "semana" : "semanas"}
              </Text>
            </View>
            <View style={[styles.calendarStatDivider, { backgroundColor: colors.border }]} />
            <View style={styles.calendarStat}>
              <Text style={[styles.calendarStatLabel, { color: colors.mutedForeground }]}>Sessões no mês</Text>
              <Text style={[styles.calendarStatValue, { color: colors.foreground }]}>{monthSessionCount}</Text>
            </View>
          </View>
          <View style={styles.weekdayRow}>
            {WEEKDAY_LABELS.map((l, i) => (
              <Text key={i} style={[styles.weekdayLabel, { color: colors.mutedForeground }]}>{l}</Text>
            ))}
          </View>
              ...
              return (
                <View key={i} style={[styles.calendarCell, muted && styles.calendarCellMuted]}>
                  <View
                    style={[
                      styles.calendarDay,
                      read && { backgroundColor: colors.volt, borderColor: colors.accentBorder },
                      !read && day.isToday && { borderColor: colors.accentText, borderWidth: 1.5 },
                      !read && !day.isToday && { backgroundColor: colors.secondary, borderColor: colors.border },
                    ]}
                  >
                    {read ? (
                      <Ionicons name="book" size={12} color={colors.accentForeground} />
                    ) : (
                      <Text
                        style={[
                          styles.calendarDayText,
                          {
                            color: muted ? `${colors.mutedForeground}66` : colors.foreground,
      ...
      <View style={styles.section}>
        <Text style={[styles.sectionTitle, { color: colors.foreground }]}>Retrospectiva</Text>
        <View style={[styles.recapCard, { backgroundColor: colors.card, borderColor: colors.border }]}>
          <View style={styles.recapHeader}>
            <View>
              <Text style={[styles.recapMonth, { color: colors.accentText }]}>{MONTH_NAMES[prevMonth]}</Text>
              <Text style={[styles.recapYear, { color: colors.foreground }]}>{prevYear}</Text>
            </View>
            <MonthlyBars
              sessions={sessions}
              year={prevYear}
      ...
      {/* Meus Cards */}
      <View style={styles.section}>
        <Text style={[styles.sectionTitle, { color: colors.foreground }]}>Meus Cards</Text>
        <TouchableOpacity
          onPress={() => router.push("/meus-cards")}
          activeOpacity={0.8}
          style={[styles.meusCardsBtn, { backgroundColor: colors.card, borderColor: colors.border }]}
        >
          ...
          <View style={styles.meusCardsText}>
            <Text style={[styles.meusCardsTitle, { color: colors.foreground }]}>Histórico de cards</Text>
            <Text style={[styles.meusCardsDesc, { color: colors.mutedForeground }]}>Cards gerados nas suas sessões</Text>
          </View>
      ...
      {featuredBadges.length > 0 && (
        <View style={styles.section}>
          <Text style={[styles.sectionTitle, { color: colors.foreground }]}>Medalhas em destaque</Text>
...
const styles = StyleSheet.create({
  ...
  sectionTitle: { fontSize: 18, fontWeight: "800", letterSpacing: -0.5, marginBottom: 14 },
  weekTrioCard: { borderRadius: 16, borderWidth: 1, padding: 18 },
  ...
  bestCard: { width: "47%", borderRadius: 14, borderWidth: 1, padding: 14, gap: 6 },
  ...
  calendarCard: { borderRadius: 16, borderWidth: 1, padding: 16, gap: 14 },
  ...
  calendarStatValue: { fontSize: 20, fontWeight: "900", letterSpacing: -0.5 },
  ...
  calendarCell: { width: `${100 / 7}%`, aspectRatio: 1, padding: 3 },
  ...
  recapCard: { borderRadius: 16, borderWidth: 1, padding: 18 },
  recapHeader: {
    flexDirection: "row",
    alignItems: "flex-end",
    justifyContent: "space-between",
  ...
  meusCardsIcon: { width: 44, height: 44, borderRadius: 12, alignItems: "center", justifyContent: "center" },
